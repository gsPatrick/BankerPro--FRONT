...
// Sessão que a API não aceita mais (token de um segredo antigo, conta removida).
export const AUTH_EXPIRED_EVENT = 'bankerpro:auth-expired';

/**
 * Token recusado pela API: apaga a sessão local e avisa quem estiver ouvindo.
 *
 * Sem isto, um token que deixou de valer deixava o app num limbo — o usuário
 * continuava "dentro" com os dados do cache, mas toda chamada falhava e as telas
 * apareciam vazias, o que parece perda de conta. O certo é mandar para o login:
 * o e-mail e a senha continuam os mesmos e nada se perde.
 *
 * Só age quando havia um token guardado, para não interferir no 401 normal de
 * "senha errada" na tela de login.
 */
function tratarSessaoInvalida(status) {
  if (status !== 401 || typeof window === 'undefined') return;
  if (!localStorage.getItem('bankerpro_token')) return;
...
export async function fetchApi(endpoint, { method = 'GET', body, token } = {}) {
  ...
  if (!response.ok) {
    ...
    tratarSessaoInvalida(response.status);
