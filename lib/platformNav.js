...
export function getVisibleNavItems(userRole = 'user') {
  const isAdmin = userRole === 'admin';
  return PLATFORM_NAV_ITEMS.filter((item) => !item.adminOnly || isAdmin);
}

export function getMobilePrimaryItems(userRole = 'user') {
  const visible = getVisibleNavItems(userRole).filter((item) => item.type !== 'divider');
...
export function getMobileSecondaryItems(userRole = 'user') {
  const primary = new Set(PLATFORM_MOBILE_PRIMARY_IDS.filter((id) => id !== 'mais'));
  return getVisibleNavItems(userRole).filter(
